...
readonly CONTAINER_NAME="llama-cpp-test"
...
lxc config device add "$CONTAINER_NAME" localai-api proxy listen=tcp:0.0.0.0:8081 connect=tcp:127.0.0.1:8081
...
echo "For this test environment, use the forwarded ports: http://localhost:8080 (web) and http://localhost:8081 (API)."
echo "To retrieve the API key for this test, run: lxc exec $CONTAINER_NAME -- cat /home/app/site/llamacpp/api_key.txt"
